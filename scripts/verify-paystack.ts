...
const EMAIL = flag("email") ?? `paystack-test-${Date.now()}@example.test`;
...
async function api(
  ...
  const envelope = await response.json().catch(() => ({ data: null, error: { message: "unreadable" } }));
...
async function main(): Promise<void> {
  ...
  if (
    !check("Paystack accepted the secret key", tested.data?.ok === true, tested.data ?? tested.error)
  ) {
  ...
  const price = wanted
    ...
    : prices.find(
        (p: any) => p.currency === "NGN" && p.active && p.unitAmount > 0 && p.model !== "USAGE_METERED"
      );
  ...
  const created = await api(
    ...
    "/v1/subscriptions",
    {
      customer: { externalId: EXTERNAL_ID, email: EMAIL, name: "Paystack Verification" },
      priceId: price.id,
      collectPayment: true,
  ...
  if (!check("the subscription was created", created.status === 201, created.error ?? created.data)) {
  ...
  check("the subscription is INCOMPLETE until it settles", created.data.subscription.status === "INCOMPLETE");
  ...
  while (Date.now() < deadline) {
    ...
    if (invoice?.status === "PAID" || subscription?.status === "ACTIVE") break;
    await sleep(3000);
  }
  ...
  if (settled) {
    check(
      ...
      { collected: invoice.amountPaid, invoiced: invoice.total }
    );
  }
  ...
  const events = (await api("GET", "/v1/webhook-events?provider=PAYSTACK&limit=10")).data;
  const items: any[] = events?.items ?? [];
  const charge = items.find((e) => e.eventType === "charge.success");

  if (
    !check("a charge.success webhook arrived", Boolean(charge), items.map((e) => e.eventType))
  ) {
    console.error(
  ...
  const methods = (await api("GET", `/v1/payment-methods?customerId=${EXTERNAL_ID}`)).data ?? [];
  const card = methods.find((m: any) => m.type === "CARD" && m.status === "ACTIVE");

  const tokenized = check("a reusable card was stored", Boolean(card), methods);
  if (tokenized) {
    note(`${card.brand ?? "card"} ending ${card.last4 ?? "????"}${card.bankName ? ` · ${card.bankName}` : ""}`);
    check("no card number was stored, only a reference", !JSON.stringify(card).includes("408408408408"));
  }

  // -- 10. The real prize ------------------------------------------------------
  section("10. Renewal with nobody present");

  if (!tokenized || !settled) {
    note("skipped — needs a settled payment with a stored card.");
  } else {
    const renewed = await api("POST", `/v1/subscriptions/${subscriptionId}/renew`, {});
    const renewalPayment = renewed.data?.payment;

    check("the next period opened", renewed.data?.renewed === true, renewed.error ?? renewed.data);
    check(
      "the stored card was charged without a checkout",
      renewalPayment?.status === "SUCCEEDED" && !renewalPayment?.checkoutUrl,
      renewalPayment
    );
    check("the subscription stayed ACTIVE", renewed.data?.subscription?.status === "ACTIVE");
  }

  // -- summary -----------------------------------------------------------------
  console.log(`\n${"─".repeat(64)}`);
  if (failed === 0) {
    console.log(`\x1b[32mAll ${passed} checks passed. Paystack is verified end to end.\x1b[0m`);
  } else {
