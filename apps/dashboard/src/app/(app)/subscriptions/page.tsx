...
export default async function SubscriptionsPage({
  ...
  const subscriptions = (await apiFetchOrNull<Subscription[]>(`/v1/subscriptions${query}`)) ?? [];
    ...
    <>
      ...
      <div className="flex flex-wrap gap-2 pb-4">
        ...
        {filters.map((filter) => (
          <Link
            key={filter}
            href={`/subscriptions?status=${filter}`}
      ...
      {subscriptions.length === 0 ? (
        <EmptyState title="Nothing here" description="No subscriptions match this filter." />
      ) : (
